...
private. Repository and environment configuration must provide
`RUNTIME_SOURCE_TOKEN`, using a fine-grained personal access token or GitHub
App token with read-only `Contents` access to the pinned Brain repository (and
the public Manager repository). Runtime checkout jobs fail at preflight with
an explicit error when this secret is unavailable; they never fall back to a
developer checkout.
...
with explicit `version` and `publish` inputs. It never builds or publishes a
local platform-specific artifact. The safe resume form is
...
The workflow signs and notarizes the application, validates Apple signatures
and the stapled ticket, runs the packaged clean-profile smoke test, and emits:

- native macOS arm64 and x64 installers;
- a signed Windows x64 installer;
- Linux x64 installers;
...
workflow downloads the release again and verifies every checksum before the
