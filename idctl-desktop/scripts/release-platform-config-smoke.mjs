...
assert.match(releaseWorkflow, /release_view\(\)/, 'release publication must retry GitHub release-record propagation');
assert.match(releaseWorkflow, /upload_release_asset\(\)/, 'release publication must retry transient asset-upload lookup failures');
assert.match(releaseWorkflow, /Failed to upload \$\(basename/, 'release publication must surface the exact asset after retry exhaustion');
