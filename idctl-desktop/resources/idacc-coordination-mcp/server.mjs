...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Read auditable IDACC task state and durable deliverables, run bounded reconciliation/evidence audit, or record already-completed external work for managed validation.',
    inputSchema: {
      type: 'object',
      required: ['action'],
      properties: {
        action: { enum: ['list', 'context', 'reconcile', 'audit-evidence', 'ingest-external'] },
        team: { type: 'string' },
        projectId: { type: 'string' },
        status: { type: 'string' },
        refs: { type: 'array', maxItems: 50, items: { type: 'string' } },
        ref: { type: 'string' },
        title: { type: 'string', description: 'Completed-work title for ingest-external.' },
        summary: { type: 'string', description: 'Bounded reporter summary for ingest-external.' },
        evidence: { type: 'array', maxItems: 20, items: { type: 'string' }, description: 'References for managed validation; they are not trusted completion proof.' },
        primaryLead: { type: 'string', description: 'Optional coordinator, for example Bob.' },
        recordId: { type: 'string', description: 'Optional idempotency/reference key for the external-work handoff.' },
